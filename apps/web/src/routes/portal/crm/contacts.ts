/**
 * CRM — Contacts  /portal/crm/contacts
 */
  ...
  async (c) => {
    ...
    try {
      const result = await db
        .prepare('SELECT * FROM contacts WHERE org_id = ? ORDER BY created_at DESC')
        .bind(orgId)
        .all();
      all = result.results ?? [];
    } catch {
    ...
    if (isApi(c)) return c.json({ contacts: all });

    const rows = all.map((ct: any) => `
      <tr class="border-b border-[var(--border)] hover:bg-[var(--surface)] transition-colors">
        <td class="px-4 py-2.5 text-sm font-medium text-[var(--text)]">${ct.name ?? '—'}</td>
        <td class="px-4 py-2.5 text-sm text-[var(--text-muted)]">${ct.account_name ?? '—'}</td>
        <td class="px-4 py-2.5 text-sm text-[var(--text-muted)]">${ct.email ?? '—'}</td>
        <td class="px-4 py-2.5 text-sm text-[var(--text-muted)]">${ct.phone ?? '—'}</td>
        <td class="px-4 py-2.5 text-sm text-[var(--text-muted)]">${ct.title ?? '—'}</td>
        <td class="px-4 py-2.5 text-sm text-[var(--text-muted)]">${ct.created_at ? new Date(ct.created_at * 1000).toLocaleDateString() : '—'}</td>
      </tr>`).join('');

    const html = `<!DOCTYPE html><html lang="en" data-theme="dark">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Contacts — EQBIS</title>
...
<div class="flex items-center justify-between mb-6">
  <div>
    ...
    <p class="text-sm text-[var(--text-muted)] mt-0.5">${all.length} contacts</p>
  </div>
  <button @click="addOpen = true" class="h-8 px-3 bg-[var(--accent)] text-white rounded text-sm font-medium hover:opacity-90">+ New Contact</button>
</div>
<div class="rounded-lg border border-[var(--border)] bg-[var(--surface)] overflow-hidden">
  <table class="w-full">
    ...
    <tbody>${rows || '<tr><td colspan="6" class="px-4 py-8 text-center text-sm text-[var(--text-muted)]">No contacts found</td></tr>'}</tbody>
  </table>
</div>
...
<div x-show="addOpen" x-cloak class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" @keydown.escape.window="addOpen = false">
  <div class="bg-[var(--surface)] border border-[var(--border)] rounded-xl w-full max-w-lg p-6" @click.outside="addOpen = false">
    ...
    <form method="POST" action="/portal/crm/contacts">
      <div class="space-y-4">
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            <label class="block text-xs font-medium text-[var(--text-muted)] mb-1">Job Title</label>
            <input name="title" class="w-full h-8 px-2.5 text-sm bg-[var(--bg)] border border-[var(--border)] rounded focus:outline-none focus:border-[var(--accent)]">
          </div>
        </div>
        <div>
          <label class="block text-xs font-medium text-[var(--text-muted)] mb-1">Account / Company</label>
      ...
      <div class="flex gap-3 mt-6">
        ...
        <button type="submit" class="flex-1 h-8 bg-[var(--accent)] text-white rounded text-sm font-medium hover:opacity-90">Create Contact</button>
      </div>
    </form>
  </div>
</div>
...
contacts.post(
  ...
  async (c) => {
    ...
    try {
      await db
        .prepare('INSERT INTO contacts (id, org_id, name, title, account_name, email, phone, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
        .bind(id, orgId, name, title, account_name, email, phone, now)
        .run();
    } catch {
    ...
    return c.redirect('/portal/crm/contacts');
  },
);
